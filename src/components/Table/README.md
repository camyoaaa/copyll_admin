...
```vue
<template>
  <s-table ref="table" size="default" :rowKey="record => record.data.id" :columns="columns" :data="loadData" :rowSelection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }"> </s-table>
</template>
